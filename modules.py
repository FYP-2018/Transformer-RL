...
def normalize(inputs, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if inside_loop:
            ...
            # initial variable inside a loop
            beta= tf.Variable(initial_value = lambda: tf.zeros(params_shape))
            gamma = tf.Variable(initial_value = lambda: tf.ones(params_shape))
        else:
...
def multihead_attention(queries, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Linear projections (? any diff between dense & just multiply
        ...
        # outputs = tf.layers.dense(outputs, num_units, use_bias=False, activation=None) 
        
        # Residual connection
        outputs += queries
              
        # Normalize
        outputs = normalize(outputs, inside_loop=inside_loop) # (N, T_q, C)
...
def feedforward(inputs, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Normalize
        # TODO: reuse the normalize for the same layer of ml and rl
        outputs = normalize(outputs, inside_loop=inside_loop)
